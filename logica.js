var model = {
	...
	fire: function(guess) {
		for (var i = 0; i < this.numShips; i++) {
		  ...
		  if (ship.hits[index] === "Golpe") {
				view.displayMessage("Golpeaste el barco antes.");
				return true;
			} else if (index >= 0) {
					ship.hits[index] = "Golpe";
					view.displayHit(guess);
					view.displayMessage("Le diste");
					if (this.isSunk(ship)) {
		...
		view.displayMessage("Intenta de nuevo");
		return false;
	},
	...
	processGuess: function(location) {
		if (location) {
			...
			if (hit && model.shipsSunk === model.numShips) {
				view.displayMessage("Hundiste todos los barcos " + this.guesses + " Bien!.");
...
function init() {
	...
	view.displayMessage("Batalla Naval");
}
